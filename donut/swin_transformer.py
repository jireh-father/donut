...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        print("partition", x_windows.shape)
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # num_win*B, window_size*window_size, C

        # W-MSA/SW-MSA
        print("partition reshape", x_windows.shape)
        attn_windows = self.attn(x_windows, mask=self.attn_mask)  # num_win*B, window_size*window_size, C
        print("attn x", attn_windows.shape)
        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C)
        print("attn x resahpe", attn_windows.shape)
        shifted_x = window_reverse(attn_windows, self.window_size, H, W)  # B H' W' C
        print("attn x shift", shifted_x.shape)
        # reverse cyclic shift
        if self.shift_size > 0:
            x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        else:
            x = shifted_x
        print("reverse cyclic shift", x.shape)
        x = x.view(B, H * W, C)
        print("reverse cyclic shift reshape", x.shape)
        # FFN
